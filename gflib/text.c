...
static u16 FontFunc_Healthbox(struct TextPrinter *);
...
static void DecompressGlyph_Healthbox(u16, bool32);
...
static u32 GetGlyphWidth_Healthbox(u16, bool32);
...
static const struct GlyphWidthFunc sGlyphWidthFuncs[] =
{
    ...
    { FONT_HEALTHBOX,   GetGlyphWidth_Healthbox },
};
...
static const struct FontInfo sFontInfos[] =
{
    ...
    [FONT_HEALTHBOX] = {
        .fontFunction = FontFunc_Healthbox,
        .maxLetterWidth = 5,
        .maxLetterHeight = 10,
        .letterSpacing = 0,
        .lineSpacing = 0,
...
static u16 FontFunc_Healthbox(struct TextPrinter *textPrinter)
{
    struct TextPrinterSubStruct *subStruct = (struct TextPrinterSubStruct *)(&textPrinter->subStructFields);

    if (subStruct->hasFontIdBeenSet == FALSE)
    {
        subStruct->fontId = FONT_HEALTHBOX;
        subStruct->hasFontIdBeenSet = TRUE;
    }
    return RenderText(textPrinter);
}
...
static u16 RenderText(struct TextPrinter *textPrinter)
{
    ...
    case RENDER_STATE_HANDLE_CHAR:
        ...
        case FONT_SHORT_NARROW:
            ...
            break;
        case FONT_HEALTHBOX:
            DecompressGlyph_Healthbox(currChar, textPrinter->japanese);
            break;
        case FONT_BRAILLE:
            break;
        }
...
static void DecompressGlyph_Healthbox(u16 glyphId, bool32 isJapanese)
{
    const u16 *glyphs;

    if (isJapanese == 1)
    {
        glyphs = gFontHealthboxLatinGlyphs + (0x100 * (glyphId >> 0x4)) + (0x8 * (glyphId & 0xF));
        DecompressGlyphTile(glyphs, gCurGlyph.gfxBufferTop);
        DecompressGlyphTile(glyphs + 0x80, gCurGlyph.gfxBufferBottom);
        gCurGlyph.width = 8;
        gCurGlyph.height = 12;
    ...
    {
        glyphs = gFontHealthboxLatinGlyphs + (0x20 * glyphId);
        gCurGlyph.width = gFontHealthboxLatinGlyphWidths[glyphId];
...
static u32 GetGlyphWidth_Healthbox(u16 glyphId, bool32 isJapanese)
{
    if (isJapanese == TRUE)
        return 8;
    else
        return gFontHealthboxLatinGlyphWidths[glyphId];
}
