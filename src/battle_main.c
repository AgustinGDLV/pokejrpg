...
static void DoBattleIntro(void)
{
    ...
    {
    case BATTLE_INTRO_STATE_GET_MON_DATA: // *TODO - documentation
    ...
    case BATTLE_INTRO_STATE_INTRO_TEXT:
        ...
        break;
    case BATTLE_INTRO_STATE_WAIT_FOR_WILD_BATTLE_TEXT: // wait for wild battle message
        if (!IsBattlerMarkedForControllerExec(GetBattlerAtPosition(B_POSITION_PLAYER_LEFT)))
            gBattleStruct->introState++;
        break;
    case BATTLE_INTRO_STATE_SET_DEX_AND_BATTLE_VARS: // set dex and battle vars
        if (!gBattleControllerExecFlags)
        {
            for (battler = 0; battler < gBattlersCount; battler++)
            {
...
static void HandleTurnActionSelectionState(void)
{
    ...
    for (battler = 0; battler < gBattlersCount; battler++)
    {
        ...
        case STATE_BEFORE_ACTION_CHOSEN: // Choose an action.
            ...
            partner = GetAliveLeftPartner(battler);
            if (partner == battler || gBattleCommunication[partner] == STATE_WAIT_ACTION_CONFIRMED)
            {
            ...
            if (!(gBattleControllerExecFlags & (((1u << battler)) | (0xF << 28) | ((1u << battler) << 4) | ((1u << battler) << 8) | ((1u << battler) << 12))))
            {
                ...
                case B_ACTION_CANCEL_PARTNER:
                        ...
                        || gBattleMons[partner].status2 & STATUS2_RECHARGE)
                    {
                        ...
                        MarkBattlerForControllerExec(battler); // *TODO - handling for multiple partners
                        return;
                    }
        ...
        case STATE_WAIT_ACTION_CONFIRMED_STANDBY:
            if (!(gBattleControllerExecFlags & (1u << battler))) // *TODO - idk what's happening here
            {
