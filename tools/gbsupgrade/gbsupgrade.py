...
def upgrade_macro(command, state):
	...
	if command_stripped.startswith('dutycycle'):
		_, arg1 = command_stripped.split()
		arg1 = parse_value(arg1)
		command = '\tduty_cycle %s\n' % (arg1)
	...
	# tone/pitch_offset
	if command_stripped.startswith('tone'):
		_, arg1 = command_stripped.split()
		arg1 = parse_value(arg1)
		command = '\tpitch_offset %s\n' % (arg1)
...
def upgrade_file(file_path):
	...
	try:
		...
		for line in lines:
			...
			# Add GBS commands
			if command.startswith('Music_'):
				spl = command.split('_')
				mus_name = spl[1]
				if len(spl) == 2:
					command = 'gbs_'+command
				if spl[-1].startswith('Ch'):
					channel_num = int(spl[-1][2])
					command = 'gbs_'+command+'\tgbs_switch '+str(channel_num-1)+'\n'+command
		...
		# Add header
		new_lines.insert(0, '\t.include \"asm/macros.inc\"\n\t.section .rodata\n\t.global gbs_Music_'+mus_name+'\n\t.align 2\n\n')
